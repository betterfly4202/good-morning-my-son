...
class VttSubtitleParser : SubtitleParser {
    ...
    override fun canParse(file: File): Boolean = file.extension == "vtt"

    override fun parse(file: File): List<SubtitleSegment> {
        logger.debug("Parsing vtt file: {}", file.name)
        ...
        while (i < lines.size) {
            val line = lines[i]
            if (line.contains("-->")) {
                val times = line.split("-->").map { it.trim().substringBefore(" ") }
                if (times.size == 2) {
        ...
        logger.debug("Parsed {} segments from vtt", segments.size)
        return segments
    }
